...
$data = new RestWs($url, $headers);
    ...
    if((isset($_GET['div'])) && ($_GET['div'] == 1)) {
        ...
        foreach ($notice as $key => $value) {
            ...
            if ($index >= 3) {
                $div .= '<div id="RTK_7dnR_'.$index.'" load-id="RTK_7dnR" style="overflow: hidden;" ></div>';
            }
            ...
            $div .= "</div>";
            if ($index >= 5) {
                $div .= '<div id="RTK_CFgp_'.$index.'" load-id="RTK_CFgp" style="overflow: hidden;" ></div> <div id="RTK_rgCe_'.$index.'" load-id="RTK_rgCe" style="overflow: hidden;" ></div>';
            }
            $creditos = str_replace($variables[4], "", $value['credit']);
            $div .= '<em class="ajax_attribution">'.$creditos.'</em>';
            $div .= $value['text'];
            if ($index >= 3) {
                $div .= '<div id="RTK_CVmP_'.$index.'" load-id="RTK_CVmP" style="overflow: hidden;" ></div>';
            }
            $div .= "</div>";
            $div .= "</div>";
            $index++;
